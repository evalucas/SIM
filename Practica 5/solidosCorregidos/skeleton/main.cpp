...
ParticleGravity				gravity(Vector3(0.0f, -10.0f, 0.0f));
...
std::vector<PhysicsObject*> gObjects;
...
void initPhysics(bool interactive)
{
	...
	PxTransform floorTransform(Vector3(0.0f, 0.0f, 0.0f));
	PxRigidStatic* floor = gPhysics->createRigidStatic(floorTransform);
	floor->attachShape(*newShape);
	gScene->addActor(*floor);
	RenderItem* floorRenderItem = new RenderItem(newShape, floor, Vector4(1.0f, 0.0f, 0.0f, 1.0f));
}
...
void stepPhysics(bool interactive, double t)
{
	...
	auto it = gObjects.begin();
	while (it != gObjects.end())
	{
		PhysicsObject* obj = (*it);
		bool kill_particle = obj->update(t);
		
		if (kill_particle)
		{
			it = gObjects.erase(it);
			forceRegistry.remove(obj);
			delete obj;
		}
		else
			++it;
...
void keyPress(unsigned char key, const PxTransform& camera)
{
	...
	case ' ':
	{
		...
		explosion = new ParticleExplosion(500000.0f, expPos, expRadius);
		explosionAge = 5.0f;
