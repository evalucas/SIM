...
extern void AddObject(PhysicsObject* obj);
...
void ParticleSystem::createNewParticle()
{
	...
	physx::PxShape* newShape = CreateShape(PxSphereGeometry(mass));
	physx::PxTransform trans(position);
	RigidBody* newParticle = new RigidBody(newShape, &trans, speed * dir, mass, age, 1.0f);
	newShape->release();
