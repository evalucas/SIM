...
extern void initBouyancyDemo();
extern void endBouyancyDemo();
extern void keyPressedBouyancyDemo(unsigned char key, const physx::PxTransform& camera);
...
void AddForceGenerator(ParticleForceGenerator* forceGen, Particle* particle) {
	forceRegistry.add(particle, forceGen);
}
...
{
	PX_UNUSED(camera);
	keyPressedBouyancyDemo(key, camera);
	...
	case ' ':
	{
		...
		explosion = new ParticleExplosion(300.0f, expPos, expRadius);
		explosionAge = 5.0f;
