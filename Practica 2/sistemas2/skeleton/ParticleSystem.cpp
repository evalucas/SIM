...
void ParticleSystem::update(float t)
{
	...
	if (time_for_next_particle < 0.0f) {
		while (time_for_next_particle < 0) {
			createNewParticle();
			time_for_next_particle += generation_rate;
		}
	}
}
float ParticleSystem::random(float _min, float _max) {
...
Vector3 Spherical2Cartesian(float r, float rho, float phi) {
...
{
	float rho = random(minRho, maxRho) * 3.14f / 180.0f;
	float phi = random(minPhi, maxPhi) * 3.14f / 180.0f;

	Vector3 dir = Spherical2Cartesian(1.0f, rho, phi);
	float speed = random(minSpeed, maxSpeed);

	float age = random(minAge, maxAge);
	...
	Particle* newParticle = new Particle(position, speed * dir, Vector3(0.0f, 0.0f, 0.0f), damping, age, newRenderItem);
	newShape->release();
